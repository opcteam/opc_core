...
import { opc_API_TYPES } from '../../common/constants/notifications';
...
export const opcAPIHandler = (response, message) => dispatch => {
  const type = _.get(message, 'type');

  if (_.isEqual(type, opc_API_TYPES.notification)) {
    const notification = _.get(message, opc_API_TYPES.notification);
