...
export const opc_LOGIN = createAsyncActionType('@auth/opc_LOGIN');
...
export const opcLogin = () => (dispatch, getState) => {
  const accessToken = Cookie.get('access_token');
  const state = getState();

  if (!getIsAuthenticated(state)) {
    return dispatch({ type: opc_LOGIN.ERROR });
  }

  const targetUsername = getAuthenticatedUserName(state);

  return dispatch({
    type: opc_LOGIN.ACTION,
    meta: targetUsername,
    payload: {
      promise: opcAPI.sendAsync('login', [accessToken]),
    },
  });
};
