...
export default function createSsrHandler(template) {
  return async function serverSideResponse(req, res) {
    try {
      ...
      if (useBeta && req.hostname === 'opc.org') {
        return res.redirect(`https://staging.opc.org${req.originalUrl}`);
      }
      ...
      return res.send(renderSsrPage(store, content, template, appUrl !== 'https://opc.org'));
    } catch (err) {
